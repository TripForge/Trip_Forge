...
import { fetchUserAsync, getUserInfo, loggedInUser, selectloggedInUser} from '../../Signup/authSlice'
import { Link } from 'react-router-dom';
import { useEffect } from 'react';
export default function UserProfile() {
  // const getUser = useSelector(selectloggedInUserToken);
  // const id = getUser.id;
  // const user = useSelector(fetchUserAsync(id));
  ...
  const dispatch = useDispatch();
    ...
    console.log("mera user hai " , user);
  ...
  return (
    <div className='p-8'>
      <div className="px-4 sm:px-0">
        <h3 className="text-base font-semibold leading-7 text-gray-900">{Age}</h3>
        <p className="mt-1 max-w-2xl text-sm leading-6 text-gray-500">{userInfo}</p>
      </div>
